...
var horasTotais = 0;
   ...
   databaseRef.orderByChild("date").once('value', function (snapshot) {
       ...
       if(horasTotais>horasLimite && localStorage.getItem("user")=="kalitianne1@gmail.com"){
        ...
        if (!protocolo_id) {
            protocolo_id = firebase.database().ref().child('trator2026').push().key;
        }
        let updates = {}
        updates["/trator2026/" + protocolo_id] = protocolo;
...
function listarfiltro() {
    ...
    databaseRef.orderByChild("date").once('value', function (snapshot) {
        ...
        snapshot.forEach(function (childSnapshot) {
            ...
            if((String(childData.cpf).includes(String(item).toUpperCase())||String(childData.nomeProdutor).includes(String(item).toUpperCase())||String(childData.rg).includes(String(item).toUpperCase())||String(childData.localidade).includes(String(item).toUpperCase())||String(childData.status).includes(String(item).toUpperCase()))){
                var row = tblUsers.insertRow(rowIndex);
            var cellNome = row.insertCell(0);
            var cellCPF = row.insertCell(1);
            var cellLocalidade = row.insertCell(2);
